...
# Allow building from either repo root or the tools directory
ARG PETEM_SRC=.

# Copy the PeTEM tools into the image
COPY ${PETEM_SRC}/ /opt/petem

# Ensure helper binaries are executable
RUN chmod +x /opt/petem/run_PeTEM.sh \
...
ENTRYPOINT ["bash", "-lc", "/opt/petem/env_check.sh && exec /opt/petem/run_PeTEM.sh"]
